...
Player* Player::create(cocos2d::Node* _listener, cocos2d::SEL_CallFunc _selector)
{
...
void Player::SetMoveEventData()
{
    ...
    if (GameManager::getInstance().
        nMap[this->_mapPos.y][this->_mapPos.x] == MapObject::WALL)
    {
        this->_mapPos = originPos;
        this->_dirState = _originDir;
        this->SetBoolDirection(_dirState);
    }
    else
        _animate->setAnimation(this->_animPlayer[_dirState]);
}
...
void Player::update(float f)
{
    ...
    // 키보드가 눌러지지 않은 상태에서도 예약된 위치까지는 움직여야 한다.
    Vec2 pos = Vec2(this->_mapPos.x * CELL_HEIGHT,
        ((MAP_HEIGHT - 1) - this->_mapPos.y) * CELL_WIDTH);
